...
print(
    f'P(Enemy Hit): {phit_enemy} - P(Keep Con): {psave} - P(Enemy Hit AND Keep Con): {pnotbreak}')
print(f'Round: {0} - Damage: {damage_hunter:.2f}')
for i in range(1, rounds):
    damage_hunter += pnotbreak**i * 2 * damage_attack
    print(
        f'Round: {i} - Damage: {pnotbreak**i * 2*damage_attack:.2f}|{damage_hunter:.2f}')
...
damage_smite = damage_dice + prob_crit/prob_hit * 2*damage_dice
